...
def obtain_tgt(client_id: str, public_registry: dict) -> dict:
    ...
    # Group AS replies by exact payload bytes; pick any bucket with >= THRESHOLD
    # signatures so one divergent authority does not break availability.
    payload_buckets = {}
    ...
    for auth_id, base_url in AS_NODES.items():
        ...
        print(f"[Client] {auth_id} signature VALID ✓")
        payload_bytes = json.dumps(ticket_payload, sort_keys=True).encode("utf-8")
        payload_key = bytes_to_b64(payload_bytes)
        bucket = payload_buckets.setdefault(payload_key, {
            "ticket_payload": ticket_payload,
            "signatures": [],
            "session_key_enc": inner_resp.get("session_key_enc"),
            "session_key_iv": inner_resp.get("session_key_iv"),
        })
        bucket["signatures"].append(sig)

    # Choose the payload bucket with most signatures.
    best_bucket = None
    for bucket in payload_buckets.values():
        if best_bucket is None or len(bucket["signatures"]) > len(best_bucket["signatures"]):
            best_bucket = bucket

    if best_bucket is None or len(best_bucket["signatures"]) < THRESHOLD:
        raise RuntimeError(
            f"Could not collect enough valid TGT signatures "
            f"(best bucket: {0 if best_bucket is None else len(best_bucket['signatures'])}/{THRESHOLD}). "
            f"Check that ≥{THRESHOLD} AS nodes are running and signing identical payloads."
        )

    print(f"\n[Client] Threshold ({THRESHOLD}) reached. TGT assembled.")

    tgt = {
        "ticket_payload":  best_bucket["ticket_payload"],
        "signatures":      best_bucket["signatures"][:THRESHOLD],
        "session_key_enc": best_bucket["session_key_enc"],
        "session_key_iv":  best_bucket["session_key_iv"]
    }
    return tgt
...
def obtain_service_ticket(client_id: str, requested_service_id: str,
    ...
    for tgs_id, base_url in TGS_NODES.items():
        ...
        print(f"[Client] {tgs_id} signature VALID ✓")
        payload_key = bytes_to_b64(st_msg_bytes)
        bucket = payload_buckets.setdefault(payload_key, {
            "payload": st_payload,
            "signatures": [],
            "service_session_key_enc": inner_resp.get("service_session_key_enc"),
            "service_session_key_iv": inner_resp.get("service_session_key_iv"),
        })
        bucket["signatures"].append(sig)
    ...
    service_ticket = {
        "payload":                   best_bucket["payload"],
        "signatures":                best_bucket["signatures"][:THRESHOLD],
        "service_session_key_enc":   best_bucket["service_session_key_enc"],
        "service_session_key_iv":    best_bucket["service_session_key_iv"],
    }
    service_session_key = aes256_cbc_decrypt(
        session_key,
        b64_to_bytes(best_bucket["service_session_key_enc"]),
        b64_to_bytes(best_bucket["service_session_key_iv"]),
    )
    return service_ticket, service_session_key
